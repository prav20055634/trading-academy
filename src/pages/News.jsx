      ...
      {usingFallback ? (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--accent-orange)" }}>
                Live feeds temporarily unavailable — showing curated market headlines
              </div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                News APIs may be blocked by your network. Click Refresh to retry.
              </div>
            </div>
          </div>
          <button className="btn btn-secondary" style={{ padding: "5px 12px", fontSize: 11 }} onClick={refetch}>
            🔄 Retry
          </button>
        </div>
      ) : (
        <div style={{
          ...
          <span style={{ fontSize: 12, color: "var(--accent-green)" }}>
            ✅ Live news loaded — {news.length} articles
          </span>
          {lastUpdate && (
            <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>
              Updated: {lastUpdate.toLocaleTimeString("en-IN")} IST
            </span>
          )}
        </div>
      )}
      ...
      <div className="grid-2" style={{ gap: 16, alignItems: "start" }}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <button className="btn btn-secondary" style={{ padding: "5px 10px", fontSize: 11 }} onClick={refetch}>
              {loading ? "⏳" : "🔄"} Refresh
            </button>
          </div>
            ...
            {[
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span className="badge" style={{ background: `rgba(${hexToRgb(s.color)},0.1)`, color: s.color, border: `1px solid ${s.color}30`, fontSize: 9 }}>
                    {s.tag}
                  </span>
        ...
        <div>
          ...
          <div className="card" style={{ padding: "0", overflow: "hidden", marginBottom: 16 }}>
            <div style={{
              display: "grid",
              gridTemplateColumns: "75px 45px 65px 1fr 70px 70px",
              gap: 4, padding: "8px 14px",
              borderBottom: "1px solid var(--border)",
              fontSize: 10, color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase"
            }}>
              <span>TIME IST</span><span>CCY</span><span>IMPACT</span><span>EVENT</span>
              <span>FORECAST</span><span>PREV</span>
            </div>
            {calendar.map((e, i) => (
              <div key={i} style={{
                display: "grid",
                ...
                <span className={`badge ${e.impact === "HIGH" ? "badge-red" : e.impact === "MEDIUM" ? "badge-orange" : "badge-blue"}`} style={{ fontSize: 9 }}>
                  {e.impact}
                </span>
          ...
          <div className="card" style={{ background: "rgba(244,67,54,0.04)", borderColor: "rgba(244,67,54,0.2)" }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--accent-red)", marginBottom: 10 }}>
              ⚠️ HIGH IMPACT NEWS — Trading Rules
            </div>
            {[
              "CLOSE all trades 30 minutes BEFORE any HIGH impact event",
              "NFP (US Jobs) — avoid EUR/USD and GBP/USD from 5:30 PM IST",
              "RBI Policy — avoid all Indian instruments 1 hour before",
              "FOMC — avoid all USD pairs from 11:00 PM IST",
              "CPI Data — expect 50–150 pip spike, stay out",
              "After news spike: wait for FVG to form THEN enter — never chase",
              "High impact news = opportunity AFTER the move, not DURING",
              "If you are IN a trade during news — move SL to breakeven minimum",
            ].map((r, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: "var(--accent-red)", minWidth: 14 }}>→</span> {r}
              </div>
            ))}
          </div>
        </div>
